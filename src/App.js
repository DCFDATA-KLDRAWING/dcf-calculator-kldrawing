...
/**
 * 版本 v4.4 更新日誌 (零警告發布版)：
 * 1. 架構重構：將 fetchHybridStockData、getTrend 等純函數移出 App 元件外部，徹底解決 React Hooks 的依賴警告。
 * 2. 順利部署：解決 Vercel 嚴格模式下因 ESLint 警告導致的 `npm run build exited with 1` 錯誤。
 * 3. 繼承所有功能：雙專家模型 Prompt、均線判定、混合資料源抓取、專屬 LOGO。
 */
...
const ASSISTANT_TOPICS = [
  ...
  "月營收年增趨勢向上",
  "月營收年增趨勢向下",
  "有缺貨新聞的個股依出現提及次數多到少排序，列出50個股號股名及新聞連結即可",
  "產業趨勢",
  "產品線分析",
  "長短期展望",
  "觀察重點",
  "利多因素",
  "可能疑慮",
  "請搜尋最新財報並以法人報告中的「情境分析」（Scenario Analysis）【保守情境】和【樂觀情境】分析下面2項，1.找出「近一年全年 EPS及預估EPS」,2.找出「接下來 5 年 預估 EPS或營收或毛利或產業趨勢的 年複合成長率 (CAGR)」。",
  "利空因素",
  "接單狀況",
  "有資本支出新聞的個股依日期近到遠，列出50個股號股名及新聞連結即可",
  "新產品",
  "景氣循環股過往正常EPS的均值",
  "相關公司",
  "同業競爭",
  ...
  "上修",
  "下修",
  "請提供 50 檔台股的股號、股名及新聞連結。請用 Markdown 表格呈現，欄位為：| 編號 | 股號 | 股名 | 新聞連結 |。請一次將 50 筆資料完整列在同一個表格中，不要中斷，也不要輸出多餘的說明文字。",
  "請列出 100個股號、股名及最新新聞連結。嚴格要求：請直接輸出名單，不要加上任何開場白、解釋或結語。請從 1 開始編號，並且務必強制輸出到第 100 個才停止。格式：[編號]. [股號] [股名] - [新聞連結]",
  "未來幾年法人預估eps成長率多少？法人會上修估值的關鍵情境是什麼?直接講重點,法人關注的正面觸發條件，法人目標價分佈，及特定投顧的評等細節",
  "券商 外資 投信 法人機構 投顧 法說會",
];

// --- 將純函數移出 App 元件外部，徹底解決 React Hooks 的依賴警告 ---
const fetchHybridStockData = async (symbol) => {
  try {
    ...
    const fetchYahoo = async (suffix) => {
      const yahooUrl = `https://query1.finance.yahoo.com/v8/finance/chart/${symbol}${suffix}?interval=1d&range=1d&_t=${Date.now()}`;
      const res = await fetchWithFallback(yahooUrl);
      if (res) {
    ...
    yahooPrice = await fetchYahoo(".TW");
    if (!yahooPrice) yahooPrice = await fetchYahoo(".TWO");
    if (!yahooPrice) return null;

    const toTimestamp = Math.floor(Date.now() / 1000);
    const fromTimestamp = toTimestamp - 180 * 24 * 60 * 60;
    const cnyesUrl = `https://ws.cnyes.com/charting/api/v1/history?resolution=D&symbol=TWS:${symbol}&from=${fromTimestamp}&to=${toTimestamp}`;

    const resCnyes = await fetchWithFallback(cnyesUrl);
    let closes = [];

    if (resCnyes) {
      const dataCnyes = await resCnyes.json();
      if (dataCnyes && dataCnyes.data && dataCnyes.data.c && dataCnyes.data.t) {
        closes = [...dataCnyes.data.c];
        const timestamps = dataCnyes.data.t;
        ...
        if (lastDateString === todayDateString) {
          closes[closes.length - 1] = yahooPrice;
        } else {
          closes.push(yahooPrice);
        }
      } else {
        closes = [yahooPrice];
      }
    ...
    return { price: yahooPrice, closes };
  } catch (e) {
    return null;
  }
};
...
const getTrend = (closes, period) => {
  if (closes.length < period + 1) {
    return {
      symbol: "-",
      tooltip: `資料天數不足 (${closes.length}/${period})`,
    };
  }
...
const calculateFairValueForSolver = (
  deltaG,
  currentRates,
  g2,
  r_val,
  ...
  currentRates.forEach((rate) => {
    multiplier *= 1 + ((Number(rate) || 0) + deltaG) / 100;
  });
  const impliedCagr = Math.pow(multiplier, 1 / 5) - 1;

  let totalPV = 0,
    currentEps = Number(eps_val) || 0;
  for (let i = 1; i <= 5; i++) {
...
const App = () => {
  ...
  const [growth1Rates, setGrowth1Rates] = useState([
    "29.95",
    "29.95",
    "10",
    "10",
  ...
  const cagrGrowth1 = useMemo(() => {
    ...
    growth1Rates.forEach((rate) => {
      multiplier *= 1 + (Number(rate) || 0) / 100;
    });
  ...
  const updateStockData = useCallback(async (stockName) => {
    ...
    const data = await fetchHybridStockData(stockCode);
    ...
    if (data) {
      ...
      setMaTrend({
        ma20: { symbol: "-", tooltip: "無法取得資料，請稍後重試" },
        ma60: { symbol: "-", tooltip: "無法取得資料，請稍後重試" },
      });
    }
...
⚠️ 規範：必須引用真實聯網數據。風格專業、冷靜、一針見血。不超過 400 字。`;
    ...
    try {
      ...
      alert(
        `已為您複製「供需派產業觀察師」專屬指令，並將跳轉至 ChatGPT！\n\n若畫面未自動帶入，請直接在對話框「貼上 (Ctrl+V)」即可。`
      );
    } catch (err) {}
    textArea.remove();
    window.open(
      `https://chatgpt.com/?q=${encodeURIComponent(systemPrompt)}`,
      "_blank"
    );
  };
...
`;
    ...
    try {
      document.execCommand("copy");
      alert(
        `已為您複製「🎯 綜合評估成長率」專屬指令，並將跳轉至 ChatGPT！\n\n若畫面未自動帶入，請直接在對話框「貼上 (Ctrl+V)」即可。`
      );
    } catch (err) {}
    textArea.remove();
    window.open(
  ...
  const handleGoogleSubmit = () => {
    ...
    let url = `https://www.google.com/search?udm=50&q=${encodeURIComponent(
      query
    )}`;
    if (dateFilter) url += `&tbs=${dateFilter}`;
  ...
  const handlePerplexitySubmit = () => {
    ...
    switch (dateFilter) {
      case "qdr:w":
        timePrompt = "請著重在「近一週」的最新資訊。";
        break;
      case "qdr:m":
        timePrompt = "請著重在「近一個月」的最新資訊。";
        break;
      case "qdr:m3":
        timePrompt = "請著重在「近三個月」的最新資訊。";
        break;
      case "qdr:m6":
        timePrompt = "請著重在「近半年」的最新資訊。";
        break;
      case "qdr:y":
        timePrompt = "請著重在「近一年」的最新資訊。";
        break;
      case "qdr:y3":
        timePrompt = "請著重在「近三年」的資訊。";
        break;
      case "qdr:y5":
        timePrompt = "請著重在「近五年」的資訊。";
        break;
      default:
        timePrompt = "";
        break;
    }
    const query = `${stockInput.trim()} ${activeTopics.join(
      " "
    )} ${timePrompt} 請詳細整理並給出重點。`;
    window.open(
  ...
  const solveForGrowth = () => {
    ...
    while (i < 50) {
      ...
      const fv = calculateFairValueForSolver(
        mid,
        growth1Rates,
        currentG2,
        currentWacc,
  ...
  const valuation = useMemo(() => {
    ...
    if (!numBaseEps)
      return { fairValue: "0.00", upside: "0.00", path: [], summary: {} };
  ...
  return (
    <div className="min-h-screen bg-slate-100 font-sans flex flex-col">
      <div className="bg-slate-800 text-white shadow-md sticky top-0 z-40 px-4 py-3 flex justify-between items-center">
        ...
        <button
          onClick={handleReset}
          className="text-[11px] text-slate-300 flex items-center gap-1 hover:text-emerald-400 transition-colors bg-slate-700/50 px-2 py-1 rounded"
        >
        ...
        <div className="max-w-3xl mx-auto px-4 py-4 space-y-3">
          <div className="grid grid-cols-7 gap-2 relative">
            ...
            <div className="col-span-2">
              <label className="text-[10px] font-bold text-indigo-500 block mb-0.5">
                每股參數 (EPS)
              </label>
              <input
                type="number"
                step="any"
                value={baseEps}
                onChange={handleInputChange(setBaseEps)}
                className="w-full bg-indigo-50 border border-indigo-200 rounded px-2 py-2 text-xs font-black text-indigo-700 text-center outline-none focus:border-indigo-500"
                placeholder="0"
              />
            </div>
            ...
            <div className="col-span-3 flex flex-col">
                ...
                <div className="flex items-center gap-1 text-[9px] font-bold px-1.5 py-0.5 bg-slate-100 rounded border border-slate-200">
                  ...
                  >
                    <RefreshCw
                      size={9}
                      className={
                        isFetchingPrice ? "animate-spin text-indigo-500" : ""
                      }
                    />
                  </button>
                </div>
              ...
              <div className="flex gap-1 h-[34px] relative">
                ...
                />
                <button
                  onClick={solveForGrowth}
                  disabled={isSolving || isFetchingPrice}
                  className="h-full bg-indigo-600 text-white px-3 rounded-r flex items-center justify-center shrink-0 active:bg-indigo-800 transition-colors"
                  title="一鍵反推隱含成長率"
                >
                  {isSolving ? (
          ...
          <div className="pt-1">
            <button
              ...
              className={`w-full flex items-center justify-center gap-1.5 py-2 rounded-lg border transition-all text-xs font-bold ${
                showAssistant
            ...
            >
              <Sparkles
                size={14}
                className={showAssistant ? "text-amber-400" : "text-amber-500"}
              />
                  ...
                  {ASSISTANT_TOPICS.map((topic, idx) => {
                    ...
                    return (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-full border text-[11px] transition-all active:scale-95 text-left leading-tight ${
                          isLongTopic ? "w-full" : ""
                        } ${
                          isSelected
                      ...
                      {activeTopics.length > 0 ? (
                        activeTopics.map((topic, idx) => (
                          <span key={idx} className="inline-flex items-center">
                            ...
                            {idx < activeTopics.length - 1 && (
                              <span className="text-slate-500 text-[10px] mx-1">
                                +
                              </span>
                            )}
                          </span>
          ...
          <div className="bg-slate-50 rounded-lg p-3 border border-slate-200 relative mt-2">
            {isSolving && (
              <div className="absolute inset-0 bg-indigo-50/60 z-10 flex items-center justify-center backdrop-blur-[1px] rounded-lg">
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="flex flex-col">
                ...
                <span className="text-[10px] text-slate-500 flex items-center gap-1 font-bold mt-0.5">
                  <Info size={12} className="text-emerald-500" /> 試算表帶入值
                  (CAGR):{" "}
              ...
              <div className="flex items-center bg-white border border-indigo-200 rounded px-2 py-1 shadow-sm">
                <span className="text-[10px] font-bold text-indigo-500 mr-1.5">
                  全設:
                </span>
                <input
                  type="number"
              ...
              {growth1Rates.map((rate, idx) => (
                <div key={idx}>
                  <input
                    ...
                    value={rate}
                    onChange={(e) =>
                      handleYearGrowthChange(idx, e.target.value)
                    }
          ...
          <div className="grid grid-cols-3 gap-3 mt-3">
            {[
              ...
              <div key={idx}>
                <label
                  className={`text-[10px] font-bold text-${item.color}-500 block mb-1 text-center`}
                >
                  {item.label}
                </label>
                <input
                  ...
                  step="any"
                  value={item.val}
                  onChange={handleInputChange(item.set)}
                  className={`w-full bg-white border border-slate-200 rounded px-2 py-2 text-xs font-black text-${item.color}-600 text-center outline-none focus:border-indigo-400`}
                  placeholder="0"
                />
              </div>
            ))}
          ...
          <div className="flex justify-between items-center pt-3 mt-3 border-t-2 border-slate-100">
            ...
            <div className="flex items-center gap-1">
              {Number(price) > 0 &&
                Math.abs(Number(valuation.fairValue) - Number(price)) < 1 && (
        ...
        <div className="flex justify-center">
          <div className="bg-white p-1 rounded-lg border border-slate-200 inline-flex shadow-sm">
            <button
              ...
              className={`flex items-center gap-1.5 px-5 py-2 rounded text-xs font-bold transition-all ${
                activeTab === "table"
              ...
              <table className="w-full text-right text-sm">
                ...
                <tbody className="divide-y divide-slate-100 text-slate-600 font-mono bg-white">
                  {valuation.path.map((row) => (
                    ...
                    >
                      <td className="px-4 py-2.5 text-center font-bold text-slate-800 bg-slate-50/50">
                        {row.year}
                      </td>
                      ...
                      <td className="px-4 py-2.5 text-emerald-600 font-bold bg-emerald-50/10">
                        {row.pv}
                      </td>
                    </tr>
                  ))}
                ...
                <tfoot className="bg-slate-50 border-t-2 border-slate-200 text-xs font-bold text-slate-600">
                  ...
                  <tr>
                    ...
                    <td className="px-4 py-3 font-mono">
                      {valuation.summary.pvTerminal} 元
                    </td>
                  </tr>
                  <tr className="bg-slate-800 text-white text-base">
                    <td
                      colSpan="3"
                      className="px-4 py-4 text-right tracking-widest"
                    >
                      股票合理價值
                    </td>
                    <td className="px-4 py-4 font-mono font-black text-emerald-400 text-lg">
                      {valuation.fairValue} 元
                    </td>
                  </tr>
                </tfoot>
              </table>
            ...
            <div className="grid grid-cols-10 gap-1.5 items-end h-56 w-full">
              {valuation.path.map((item, index) => {
                ...
                const heightPercentage = Math.max(
                  10,
                  (parseFloat(item.eps) / maxEps) * 100
                );
                  ...
                  >
                    ...
                    <div className="w-full relative flex items-end justify-center h-full">
                      <div
                        ...
                        } ${parseFloat(item.eps) < 0 ? "bg-rose-300" : ""}`}
